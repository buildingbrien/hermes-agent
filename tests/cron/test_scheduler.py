...
class TestDeliverResultWrapping:
    ...
    def test_delivery_wraps_content_with_header_and_footer(self):
        ...
        assert "daily-report — scheduled update" in sent_content
        # Raw job ids must never reach the customer (no-mechanics rule).
        assert "job_id" not in sent_content
        assert "-------------" in sent_content
        assert "Here is today's summary." in sent_content
        assert "To pause or change this" in sent_content
    ...
    def test_delivery_uses_job_id_when_no_name(self):
        ...
        sent_content = send_mock.call_args.kwargs.get("content") or send_mock.call_args[0][-1]
        assert "abc-123 — scheduled update" in sent_content
